...
class RotaryEncoder:
   ...
   The following example will logging.debug a Rotary Encoder change direction
   ...
   def __init__(self, pinA, pinB, pi_config, publish_topics_config, broker_connection, publish_callback_handler, pull_up=False):
       """
       ...
       self.publish_topics_config = publish_topics_config
       self.broker_connection = broker_connection
       self.when_rotated = self.on_rotate
       self.publish_callback_handler = publish_callback_handler
   ...
   def on_rotate(self, value):
      logging.debug('Speed in on rotate : {}'.format(value));
      speed = self.publish_callback_handler.get_speed()
      if value > 0:
         if speed + 2.5 <= 100.0:
            speed += 2.5
            self.publish_callback_handler.set_speed(speed)
      if value < 0:
         if speed >= 2.5:
            speed -= 2.5
            self.publish_callback_handler.set_speed(speed)

      logging.debug('New Speed after rotary dial turn: {}'.format(speed))
      # Send the speed message
      msg = "{\"gateway-name\": \"" + self.pi_config.get_name() + "\", "
      msg += "\"version\": 1, "
      msg += "\"system-time-ms-local\": " + str(time.time() * 1000) + ", "
      ...
      self.broker_connection.publish(msg, self.publish_topics_config.get_well_pump_speed(), qos=0, retain=False, check_for_completion=False)
      if self.pi_config.get_print_debug_messages():
         logging.debug('Well Pump Speed Msg: '.format(msg))
...
def main():
   ...
   rotary = RotaryEncoder(18, 27, config_reader.get_pi_config(), config_reader.get_publish_topics_config(), 
                          mqtt_broker_connection, publish_callback_handler)
