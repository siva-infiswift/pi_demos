...
class PublishCallbackHandler:
   ...
   def handle(self, msg):
      ...
      if msg.topic == subscribe_topics_config.get_analytics_engine_fill_rate():
         if 'fill-rate' in parsed_json:
            self.fill_rate = float(parsed_json['fill-rate'])
            logging.debug("Fill rate is: {}".format(self.fill_rate))
      if msg.topic == subscribe_topics_config.get_knockout_tank_oil_volume():
...
def main():
   # Initialize log file
   log_filename = '/tmp/MqttTest_{date:%Y_%m_%d_%H:%M:%S}.log'.format( date=datetime.datetime.now() )
